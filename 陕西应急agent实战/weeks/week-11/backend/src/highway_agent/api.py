...
def create_app(
    ...
    @app.post("/api/agents/safety-review/invoke", response_model=SafetyReviewResult)
    async def invoke_safety_review(request: SafetyReviewRequest) -> SafetyReviewResult:
        """复核引用、证据时效、动作权限和提示词注入风险。"""

        return SafetyReviewAgent().invoke(request)

    @app.post("/api/agents/supervisor/invoke", response_model=SupervisorResult)
    async def invoke_supervisor(request: SupervisorRequest) -> SupervisorResult:
        ...
        app.state.metrics.supervisor_invocations.labels(status=result.status).inc()
        return result
