...
@pytest.mark.asyncio
async def test_tool_client_short_circuits_after_three_failures() -> None:
    """连续失败后不再访问上游，统一返回熔断错误。"""
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            503,
            json={"detail": {"error_code": "UPSTREAM_DOWN", "message": "故障"}},
        )
    ...
    blocked = await tools.query_road_status("G65", "QINLING-01")
